...
export async function importHistoryRange(db, symbol, from, to) {
  ...
  const t = db.prepare('SELECT id, ticker, exchange FROM tickers WHERE UPPER(ticker)=?').get(symbol.toUpperCase())
  if (!t) {
    throw new Error(`Ticker ${symbol} no existe en la tabla tickers`)
  }

  // Crear objeto con exchange para APIs
  const tickerObj = { ticker: t.ticker, exchange: t.exchange || 'NYSE' }

  // Pasar objeto con exchange a fetchDailyHistory
  const { items, source, attempts = [] } = await fetchDailyHistory(tickerObj, from, toDate)
  if (!items || items.length === 0) {
    ...
    try {
      // Pasar objeto con exchange a fetchPriceForSymbol
      const latest = await fetchPriceForSymbol(tickerObj)
