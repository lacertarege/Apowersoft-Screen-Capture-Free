...
export async function backfillFxJob(db, fullMode = false) {
  ...
  if (fullMode) {
    ...
    ) SELECT d FROM dates WHERE d NOT IN (SELECT fecha FROM tipos_cambio WHERE fuente_api IS NOT NULL)`).all(hoyLima).map(r => r.d)
    ...
    dates = db.prepare(`WITH RECURSIVE dates(d) AS (
      ...
      UNION ALL
      SELECT DATE(d,'+1 day') FROM dates WHERE d < DATE(?)
    ) SELECT d FROM dates WHERE d NOT IN (SELECT fecha FROM tipos_cambio WHERE fuente_api IS NOT NULL)`).all(hoyLima, hoyLima).map(r => r.d)
