...
export default function DualAxisLineChart({ data, currency = 'USD', width = null, height = 400, padding = 50 }) {
  ...
  const rentabilidadValues = data.map(d => d.rentabilidadPorcentaje)

  const minInversion = Math.min(...inversionValues, 0)
  const maxInversion = Math.max(...inversionValues)
  const minRentabilidad = Math.min(...rentabilidadValues)
  const maxRentabilidad = Math.max(...rentabilidadValues)

  const inversionRange = maxInversion - minInversion || 1
  const rentabilidadRange = maxRentabilidad - minRentabilidad || 1
  ...
  const yForRentabilidad = (value) => height - paddingBottom - ((value - minRentabilidad) / rentabilidadRange) * (height - paddingTop - paddingBottom)
  ...
  const rentabilidadPoints = data.map((d, i) => ({ x: xFor(i), y: yForRentabilidad(d.rentabilidadPorcentaje) }))
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%' }}>
      <svg width={chartWidth} height={height} viewBox={`0 0 ${chartWidth} ${height}`} style={{ display: 'block', width: '100%', height: 'auto' }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(ratio => {
          const val = minRentabilidad + ratio * rentabilidadRange
          return <text key={`r-${ratio}`} x={chartWidth - paddingRight + 10} y={yForRentabilidad(val) + 4} fontSize="11" fill="#4b5563" textAnchor="start">{val.toFixed(1)}%</text>
        })}
        {data.map((d, i) => {
        ...
        <path d={createPath(rentabilidadPoints)} fill="none" stroke="#10b981" strokeWidth="1.5" strokeDasharray="5,3" />
        {hoverPoint !== null && (
          <g>
            ...
            <text x={Math.max(100, Math.min(chartWidth - 90, xFor(hoverPoint)))} y={86} fontSize="11" fill={data[hoverPoint].rentabilidadPorcentaje >= 0 ? "#4ade80" : "#fb7185"} textAnchor="middle" fontWeight="600">Rent: {data[hoverPoint].rentabilidadPorcentaje.toFixed(2)}%</text>
          </g>
        )}
        ...
        <text x={chartWidth - 15} y={height / 2} fontSize="12" fill="#374151" fontWeight="700" transform={`rotate(90, ${chartWidth - 15}, ${height / 2})`} textAnchor="middle">Rentabilidad (%)</text>
      </svg>
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '10px', fontSize: '12px', color: '#6b7280' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '16px', height: '3px', backgroundColor: '#10b981', borderTop: '1px dashed #10b981' }}></div><span>Rentabilidad (%)</span></div>
      </div>
    </div>
  )
}
